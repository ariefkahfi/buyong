...
class Pegawai_model extends CI_Model{ 
	...
	/* GET DAT APEGAWAI */
		...
		function cekMasuk(){
			...
			$datenow=date("Y-m-d");
			$jammasuk="";
			$ceknippos=$this->input->post('nippos');
			$cekid    =$this->session->userdata('id_user');
			if($ceknippos != $cekid ) {
				echo'<hr><label style="font-size:40px;font-family:calibri"> ANDA SUDAH MELAKUKAN ABSENSI </label>';
				return false;
			}
